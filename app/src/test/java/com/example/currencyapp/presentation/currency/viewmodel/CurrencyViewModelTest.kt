...
class CurrencyViewModelTest {
    ...
    private lateinit var outComeResponse : Outcome<OperationResult<CurrencyListResult>>
 ...
 //        currencyListUseCase =mock(CurrencyListUseCase.class)
        currencyViewModel = CurrencyViewModel(currencyListUseCase)
    }
    ...
    fun testGetCurrencyData() {
        ...
        val dataResponse = Gson().fromJson(currencyStringData,
         CurrencyListResult::class.java)

         outComeResponse = Outcome(MutableStateFlow(OperationResult.SuccessOperationResult(dataResponse)))
        runBlocking {
           `when`(currencyListUseCase.getCurrencyData("USD",currencyViewModel.viewModelScope))
                .thenReturn(outComeResponse)
            currencyListUseCase.getCurrencyData("USD",currencyViewModel.viewModelScope)
            currencyViewModel.onGetCurrencySuccessLD.value?.let { Assert.assertTrue(it.success) }
        }
    }

  /*  @Test
    fun testGetCurrencyDataError(){
        val  error1 : Error = Error.Companion.getError(9000)
        outComeResponse = Outcome(MutableStateFlow(OperationResult.ErrorOperationResult(error1)))
        runBlocking {
            `when`(currencyListUseCase.getCurrencyData("AAA",currencyViewModel.viewModelScope))
                .thenReturn(outComeResponse)
            currencyListUseCase.getCurrencyData("AAa",currencyViewModel.viewModelScope)
            currencyViewModel.onErrorLD.value?.let { Assert.assertTrue(it.responseError?.getMessage() == "unknown_error_msg") }
        }
    }*/
